...
function SkinService:BuySkin(player, characterName, skinName)
	...
	if price > 0 then
		...
		if currency == "Col" then
			spent, message = self.CurrencyService:SpendCol(player, price, "BuySkin:" .. characterName .. ":" .. skinName)
		elseif currency == "SeedShards" then
			spent, message = self.CurrencyService:SpendSeedShards(player, price, "BuySkin:" .. characterName .. ":" .. skinName)
		else
