...
function CharacterService:BuyCharacter(player, characterName)
	...
	local currency = character.Currency or "Coins"

	if price > 0 then
		local spent, message
		if currency == "Coins" then
			spent, message = self.CurrencyService:SpendCoins(player, price, "BuyCharacter:" .. characterName)
		elseif currency == "Gems" then
			spent, message = self.CurrencyService:SpendGems(player, price, "BuyCharacter:" .. characterName)
		else
